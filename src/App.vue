...
const intakeForm = reactive({ contactName: '', contactEmail: '', notes: '', members: [] })
...
const compiled = ref({ inputs: [], sockets: [], outputs: { mains: [], monitors: [], warnings: [] }, warnings: [], commands: [] })
...
const compile = async () => {
  if (!currentSlot.value) return compiled.value = { inputs: [], sockets: [], outputs: { mains: [], monitors: [], warnings: [] }, warnings: [], commands: [] }
  currentSlot.value.channelOverrides ||= {}
  compiled.value = await request('/api/plan/compile', { method: 'POST', body: JSON.stringify({ members: currentSlot.value.members, patches: currentSlot.value.patches, channelOverrides: currentSlot.value.channelOverrides }) })
...
const syncEvent = async () => { syncing.value=true; try { const result=await request('/api/events/'+currentEvent.value.id+'/sync',{method:'POST'}); await reload(); currentEvent.value=await request('/api/events/'+currentEvent.value.id); notify(result.scenes.length+' X32 scenes created') } catch(error){ notify(error.message) } finally{ syncing.value=false } }
const syncArtistScene = async () => { if(!currentSlot.value)return; syncing.value=true; try { const result=await request('/api/events/'+currentEvent.value.id+'/bands/'+currentSlot.value.id+'/sync',{method:'POST',body:JSON.stringify({mode:'scene'})}); await reload(); currentEvent.value=await request('/api/events/'+currentEvent.value.id); notify('Scene '+result.result.sceneSlot+' synced for '+result.result.bandName) } catch(error){ notify(error.message) } finally{ syncing.value=false } }
const syncArtistChannels = async () => { if(!currentSlot.value)return; syncing.value=true; try { const result=await request('/api/events/'+currentEvent.value.id+'/bands/'+currentSlot.value.id+'/sync',{method:'POST',body:JSON.stringify({mode:'channels'})}); notify(result.result.commandCount+' channel commands applied for '+result.result.bandName) } catch(error){ notify(error.message) } finally{ syncing.value=false } }
const syncInputChannel = async input => { if(!currentSlot.value)return; syncing.value=true; try { const result=await request('/api/events/'+currentEvent.value.id+'/bands/'+currentSlot.value.id+'/inputs/'+input.id+'/sync',{method:'POST'}); notify('CH '+String(input.channel).padStart(2,'0')+' applied ('+result.result.commandCount+' commands)') } catch(error){ notify(error.message) } finally{ syncing.value=false } }
...
onMounted(async () => {
  if (isIntake) {
    try { intake.value=await request('/api/intake/'+token); Object.assign(intakeForm,intake.value.band); intakeForm.members=(intakeForm.members.length?intakeForm.members:[member()]).map(normalizeMember) }
  ...
  <div v-if="isIntake" class="intake-shell">
    ...
    <main v-else-if="intake" class="intake-main">
      ...
      <form @submit.prevent="submitIntake">
        ...
        <section class="form-section"><b>03</b><div><h2>Stage notes</h2><label>Anything unusual?<textarea v-model="intakeForm.notes" rows="4"></textarea></label></div></section>
