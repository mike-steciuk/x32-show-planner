...
export async function loadStore() {
  ...
  try {
    ...
    for (const event of store.events ?? []) {
      ...
      event.runOfShow ||= []
      for (const slot of event.bands ?? []) slot.channelOverrides ||= {}
    }
    return store
  } catch (error) {
...
export function publicBandSlot(event, slot) {
  return {
    ...
    band: { name: slot.bandName, contactName: slot.contactName ?? '', contactEmail: slot.contactEmail ?? '', members: slot.members, notes: slot.notes },
